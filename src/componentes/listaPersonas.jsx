...
import validaciones from '../servicios/validaciones'; // Ajusta la ruta según la ubicación de tu archivo JSON
...
const ListaPersonas = () => {
    ...
    useEffect(() => {
        setPersonas([...validaciones.Telefonos, ...validaciones.tarjetas]);
    }, [validaciones.Telefonos, validaciones.tarjetas]);
    const handleBack = () => {
        navigate("/cajero");
    };
                    ...
                    <li key={index} className="bg-white shadow-md rounded-lg p-4 border">
                        {persona.tarjeta ? (
                            ...
                            <div className="text-gray-700">
                                ...
                                <strong className="text-lg">Saldo:</strong> {persona.clave.saldo ? `$${persona.clave.saldo}` : 'No disponible'}
                                <br />
                                <strong className="text-lg">Bloqueada:</strong> {persona.clave.bloqueada == true ? 'Sí' : 'No'}
                            </div>
                        )}
                    </li>
                ))}
